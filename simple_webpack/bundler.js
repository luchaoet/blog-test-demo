...
function createGraph(entry) {
  ...
  for (const asset of queue) {
    ...
    asset.dependencies.forEach((relativePath) => {
      ...
      // 避免多次引用导致的重复生成子依赖
      const index = queue.findIndex((item) => item.filename === absolutePath)
      const child = index === -1 ? createAsset(absolutePath) : queue[index]
      // 记录子依赖项
      asset.mapping[relativePath] = child.id
      // 将新的子依赖也加入队列中，继续遍历
      if (index === -1) queue.push(child)
    })
  }
  return queue
}
...
// 打包生成文件
fs.writeFileSync('./build/index.js', ret)
